...
const ApiRequestOverlay: React.FC<{
  open?: boolean;
  ...
  useEffect(() => {
    setRequest(getApiLog().request);
    return subscribeApiLog((req) => setRequest(req));
  }, []);
  return (
    <div
        ...
        {open && (
          <div className="pt-4 px-[18px] pb-3">
            {request ? (
              ...
              <div className="text-slate-300 text-[14px]">No request yet</div>
            )}
          </div>
        )}
      </div>
