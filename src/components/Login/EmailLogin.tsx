...
export default () => {
	...
	const sendOTPFlow = async (mode: Mode) => {
		let res: any;
		if (mode === "api") {
			res = await authClient.requestOTPForEmail(baseUrl, email, clientPK, clientSWA);
		}
		else if (mode === "sdk") {
	...
	const verifyOTPFlow = async (mode: string) => {
		...
		let res: any;
		if (mode === "api") {
			const idTokenRes = await authClient.verifyOTPForEmail(baseUrl, email, otp, token, clientPK, clientSWA);
			const idToken = idTokenRes.auth_token;
			console.log("id token", idToken);
			const provider = "okto";
			res = await authClient.authenticate(baseUrl, idToken, provider, clientPK, clientSWA);
